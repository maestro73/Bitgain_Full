...
import datetime
#TODO 1. Output results to csv
#TODO 2. Binance Trading API
#TODO 3. Live TA Analysis
#TODO 4. Alt-coin data
...
def buildNet(w_init="glorot_uniform",act="tanh"):
    ...
    net = Sequential()
    net.add(Dense(12,kernel_initializer=w_init,input_dim=12,activation='linear')) #linear-->relu
...
def chart(real,predicted,show=True):
    ...
    plt.ylabel('BTC/USD')
    plt.xlabel("9Minute Intervals")
    plt.savefig("chart_%s_%s.png" % (epochs, today))
    if show:plt.show()
...
if __name__ == '__main__':
    ...
    if args.run is not None:
        ...
        print("trained!\nSaving...",end="")
        net.save_weights("model_%s_%s.h5" % (epochs, today))
        print("saved!")
